  ...
  if (!hasPosition) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground/70">输入成本价和股数，即可追踪盈亏</p>
        <Button
          variant="outline"
          onClick={onEditClick}
        ...
        <div className="mt-8 border-t border-border/30 pt-6">
          ...
          <div className="mt-2 flex items-center justify-center gap-2">
            ...
            <span className={`text-sm ${isStockUp ? "text-stock-up" : "text-stock-down"}`}>
              {isStockUp ? "+" : ""}{change.toFixed(2)} ({isStockUp ? "+" : ""}{changePercent.toFixed(2)}%)
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center py-4">
      ...
      <div className="mt-6 grid w-full max-w-sm grid-cols-3 gap-4 rounded-xl bg-secondary/30 p-4">
        ...
        <div className="text-center">
          <p className="text-xs text-muted-foreground">持股数量</p>
          <p className="mt-1 font-mono text-sm font-medium text-foreground">
            {sharesNum.toLocaleString()}股
          </p>
        </div>
      </div>
